...
export async function onRequest(context){
  ...
  if((url.pathname==="/news"||url.pathname==="/news.html")&&request.method==="GET"){
    ...
    const patch=`<script>(()=>{const n=s=>String(s||'').replace(/\\s+/g,' ').trim().toUpperCase();const apply=()=>{const title=[...document.querySelectorAll('h1,h2,h3,h4,div,span')].find(x=>n(x.textContent)==='BÀI VIẾT MỚI NHẤT');if(!title)return false;title.style.marginTop='';let section=title.parentElement;while(section&&section!==document.body){const cs=getComputedStyle(section);if(cs.borderTopWidth!=='0px'||cs.borderLeftWidth!=='0px'||cs.backgroundColor!=='rgba(0, 0, 0, 0)')break;section=section.parentElement;}if(!section)return false;const prev=section.previousElementSibling;if(prev){prev.style.marginBottom='0';prev.style.paddingBottom='24px';}section.style.position='relative';section.style.clear='both';section.style.marginTop='0';section.style.paddingTop='24px';return true;};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{if(!apply())setTimeout(apply,300)},{once:true});else if(!apply())setTimeout(apply,300);})();<\/script>`;
