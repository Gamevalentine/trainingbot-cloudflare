...
const PROFILES:Profile[]=[
  ...
  {slug:'hoc-tap',tag:'Học tập',terms:['học tập','giáo dục','khóa học','ôn thi','education','learning','course','study','exam','quiz','flashcard'],primaryUse:'Học tập, ôn luyện hoặc tiếp cận tài liệu và nội dung giáo dục trên web.',audience:'Người học, sinh viên hoặc người cần tự học trực tuyến.'},
...
function inferProfile(text:string){let best:Profile|null=null,bestScore=0;for(const profile of PROFILES){const score=termScore(text,profile.terms);if(score>bestScore){best=profile;bestScore=score;}}return bestScore>=4?best:null;}
function inferTags(text:string){return TAG_RULES.filter((rule)=>termScore(text,rule.terms)>=2).sort((a,b)=>termScore(text,b.terms)-termScore(text,a.terms)).slice(0,5).map((rule)=>rule.name);}
...
export async function importWebsite(rawUrl:string,categoryId:number|null=null){
  ...
  const contentType=String(response.headers.get('content-type')||'').toLowerCase();const html=contentType.includes('text/html')?await readLimitedText(response):'';const meta=html?metadata(html,url):{title:'',siteName:'',description:'',keywords:'',image:'',icon:new URL('/favicon.ico',url).toString()};
  const name=(meta.siteName||meta.title||url.hostname.replace(/^www\./,'')).slice(0,160),slug=await uniqueSlug(name),description=meta.description.slice(0,1000);
  const context=normalizeSearch([meta.siteName,meta.title,meta.title,description,description,meta.keywords,url.hostname,html?pageText(html):''].join(' '));
  ...
  const inferred=inferProfile(context),profile=(selectedCategory&&PROFILES.find((p)=>p.slug===selectedCategory.slug))||inferred;
  ...
  const finalCategory=selectedCategory||inferredCategory,tagNames=inferTags(context);if(!tagNames.length&&(finalCategory?.name||profile?.tag))tagNames.push(finalCategory?.name||profile!.tag);
