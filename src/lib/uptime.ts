...
type ServiceBinding = { fetch(input:Request):Promise<Response> };
export type UptimeOptions = { lumaService?:ServiceBinding };
...
function isLuma(url:string) {
  try { return new URL(url).hostname.toLowerCase()==='luma-v33.ai-vn.workers.dev'; }
  catch { return false; }
}
async function probeService(url:string,service:ServiceBinding) {
  const started=Date.now();
  try {
    const response=await service.fetch(new Request(url,{method:'GET',headers:{Accept:'text/html,*/*;q=0.8','User-Agent':'ToolpickBot/1.0'}}));
...
async function nextAutoStatus(database:any,app:AppProbe,isUp:boolean) {
  if (app.status!=='active'&&app.status!=='offline') return app.status;
  if (isUp) return 'active';
  const rows=(await database.prepare('SELECT is_up FROM uptime_checks WHERE app_id=? ORDER BY checked_at DESC,id DESC LIMIT 2').bind(app.id).all()).results as {is_up:number}[];
  return rows.length>=2&&rows.every((row)=>row.is_up===0)?'offline':app.status;
}

export async function checkAppUptime(database:any,app:AppProbe,options:UptimeOptions={}):Promise<UptimeResult> {
  const checked_at=new Date().toISOString();
  const probe=isLuma(app.production_url)&&options.lumaService
    ? await probeService(app.production_url,options.lumaService)
    : await probeUrl(app.production_url);
  ...
  const nextStatus=await nextAutoStatus(database,app,probe.is_up);
...
export async function checkAllUptime(database:any,limit=50,options:UptimeOptions={}) {
  ...
  for(let i=0;i<apps.length;i+=4) {
    const batch=await Promise.all(apps.slice(i,i+4).map((app)=>checkAppUptime(database,app,options)));
    results.push(...batch);
  }
  return results;
}
