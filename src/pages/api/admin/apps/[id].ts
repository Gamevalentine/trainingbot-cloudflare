...
export const POST: APIRoute = async ({ request, params, url }) => {
  ...
  try {
    ...
    await db().prepare(`UPDATE apps SET name=?,slug=?,production_url=?,admin_url=?,github_repo=?,cloudflare_project=?,short_description=?,description=?,primary_use=?,audience=?,features=?,category_id=?,status=?,visibility=?,featured=?,pricing_type=?,platform=?,tech_stack=?,version=?,search_keywords=?,monitor_enabled=?,updated_at=? WHERE id=?`).bind(data.name,data.slug,data.production_url,data.admin_url,data.github_repo,data.cloudflare_project,data.short_description,data.description,data.primary_use,data.audience,data.features,data.category_id,data.status,data.visibility,data.featured,data.pricing_type,data.platform,data.tech_stack,data.version,data.search_keywords,data.monitor_enabled,data.updated_at,id).run();
