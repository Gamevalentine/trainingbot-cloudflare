...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const result = await db().prepare(`INSERT INTO apps (name,slug,production_url,admin_url,github_repo,cloudflare_project,short_description,description,primary_use,audience,features,category_id,status,visibility,featured,pricing_type,platform,tech_stack,version,search_keywords,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(data.name,data.slug,data.production_url,data.admin_url,data.github_repo,data.cloudflare_project,data.short_description,data.description,data.primary_use,data.audience,data.features,data.category_id,data.status,data.visibility,data.featured,data.pricing_type,data.platform,data.tech_stack,data.version,data.search_keywords,data.updated_at).run();
