(()=>{
  ...
  function toast(message){document.querySelector('.tb-toast')?.remove();const el=document.createElement('div');el.className='tb-toast';el.textContent=message;document.body.appendChild(el);setTimeout(()=>el.remove(),3200);}
  ...
  function render(posts){
    ...
    grid.innerHTML=posts.map(post=>{
      ...
      return `<article class="tb-player-card" data-id="${escapeHtml(post.id)}">
        ...
        <div class="tb-player-actions"><button class="tb-discord-btn" type="button" data-contact>☁ Liên hệ Discord</button><button class="tb-copy-btn" type="button" data-copy>▣ Sao chép</button></div>
  ...
  form?.addEventListener('submit',async e=>{
    e.preventDefault();
    const submit=form.querySelector('button[type="submit"]');submit.disabled=true;submit.textContent='ĐANG ĐĂNG...';
    ...
    try{
      ...
      statusEl.textContent='Đăng tin thành công.';statusEl.className='tb-match-form-status success';
      form.reset();
      setTimeout(()=>{closeModal();load();toast('Tin tìm đồng đội đã được đăng.');},450);
    }catch(err){statusEl.textContent=err.message||'Không thể đăng tin.';statusEl.className='tb-match-form-status error';}
    finally{submit.disabled=false;submit.textContent='ĐĂNG TIN TÌM ĐỒNG ĐỘI →';}
  });

  async function closeOwn(id){
    const token=ownTokens()[id];if(!token)return;
    if(!confirm('Đánh dấu tin này là đã tìm đủ đồng đội?'))return;
    try{const res=await fetch(API,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:'close',id,manage_token:token})});const data=await res.json();if(!res.ok||!data.ok)throw new Error(data.message||'Không cập nhật được tin.');removeToken(id);toast('Đã ẩn tin tìm đồng đội.');load();}catch(err){toast(err.message||'Không cập nhật được tin.');}
  }
