...
export default function Page(){
  ...
  return <div className="shell">
    <Top profile={profile}/>
    <div className="app">
      <Nav profile={profile} view={view} setView={setView}/>
...
function Top({profile}:{profile:Profile}){return <header className="topbar"><div className="brand"><span>♥</span>Kết Nối</div><div className="top-note"><b>Hẹn hò</b><span>Tìm một người hợp với bạn</span></div><div className="user-mini"><div><strong>{profile.display_name}</strong><div className="muted">{profile.city||'Chưa đặt địa điểm'}</div></div><div className="avatar">{initials(profile.display_name)}</div></div></header>}

function Nav({profile,view,setView}:{profile:Profile;view:View;setView:(v:View)=>void}){
  const items:[View,string,string][]=[['discover','✦','Khám phá'],['matches','♥','Kết nối'],['chat','●','Tin nhắn'],['profile','◉','Hồ sơ']]
  return <aside className="side"><div className="side-profile"><div className="side-avatar">{initials(profile.display_name)}</div><div><strong>{profile.display_name}</strong><span>{profile.city||'Chưa đặt địa điểm'}</span></div></div><div className="side-title"><span>HẸN HÒ</span><b>Dành cho bạn</b></div><div className="nav">{items.map(([v,icon,t])=><button key={v} className={view===v?'active':''} onClick={()=>setView(v)}><i>{icon}</i><span>{t}</span></button>)}<button className="logout" onClick={()=>void supabase.auth.signOut()}><i>↗</i><span>Đăng xuất</span></button></div><div className="side-foot">Chỉ hiển thị hồ sơ của người từ 18 tuổi và chỉ mở chat sau khi cả hai cùng thích nhau.</div></aside>
}
...
function RightRail({me}:{me:Profile}){return <aside className="right-rail"><section className="rail-card warm"><span className="rail-kicker">HỒ SƠ CỦA BẠN</span><h3>Đang tìm {lookingText(me.looking_for)}</h3><p>{me.city||'Chưa đặt địa điểm'} · {genderText(me.gender)}</p><div className="rail-progress"><span style={{width:'84%'}}/></div><small>Hồ sơ đã sẵn sàng để được khám phá.</small></section><section className="rail-card"><div className="rail-icon">♡</div><h3>Hẹn hò có chủ đích</h3><p>Một lời giới thiệu rõ ràng và ảnh chân thật giúp cuộc trò chuyện bắt đầu tự nhiên hơn.</p></section><section className="rail-card safety"><div className="rail-icon">✓</div><h3>An toàn trước tiên</h3><p>Không gửi tiền, mật khẩu hay mã xác minh cho người mới quen. Hãy gặp ở nơi công cộng trong lần đầu.</p></section></aside>}
...
function Matches({user,openChat}:{user:User;openChat:(m:Match)=>void}){
  ...
  return <section><div className="section-heading"><div><span className="eyebrow">CẢ HAI CÙNG THÍCH</span><h2>Kết nối của bạn</h2><p>Mở một cuộc trò chuyện khi bạn đã sẵn sàng.</p></div><div className="suggest-count"><b>{matches.length}</b><span>kết nối</span></div></div><div className="list">{matches.length===0?<div className="card empty rich-empty"><div className="empty-heart">♡</div><h3>Chưa có kết nối</h3><p>Khi hai người cùng thích nhau, kết nối sẽ xuất hiện ở đây.</p></div>:matches.map(m=><button key={m.id} className="row" onClick={()=>openChat(m)}>{m.photo?<img className="avatar" src={m.photo} alt=""/>:<div className="avatar">{initials(m.other?.display_name)}</div>}<span className="grow"><strong>{m.other?.display_name||'Thành viên'}</strong><span className="muted">Nhấn để trò chuyện</span></span><span className="row-arrow">›</span></button>)}</div></section>
}
