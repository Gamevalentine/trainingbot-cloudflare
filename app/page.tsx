...
type Message = { id:number; match_id:string; sender_id:string; body:string; created_at:string }
type View = 'discover'|'matches'|'chat'|'profile'
...
async function signed(path:string|null|undefined){ if(!path)return null; const {data}=await supabase.storage.from('profile-photos').createSignedUrl(path,3600); return data?.signedUrl||null }
...
export default function Page(){
  ...
  return <div className="shell"><Top profile={profile}/><div className="app"><Nav view={view} setView={setView}/><main className="content">
    {view==='discover'&&<Discover user={session.user} me={profile}/>} 
    {view==='matches'&&<Matches user={session.user} openChat={openChat}/>} 
    {view==='chat'&&(chatMatch?<Chat user={session.user} match={chatMatch}/>:<Matches user={session.user} openChat={openChat}/>)}
    {view==='profile'&&<ProfileEditor user={session.user} profile={profile} onSaved={()=>loadProfile(session.user)} embedded/>}
  </main></div><MobileNav view={view} setView={setView}/></div>
}
...
function Auth(){
  ...
  return <div className="auth-wrap"><form className="auth-card" onSubmit={submit}><div className="brand"><span>♥</span>Kết Nối</div><h1>{signup?'Tạo tài khoản':'Đăng nhập'}</h1><p>Hẹn hò an toàn hơn: chỉ trò chuyện khi hai người cùng thích nhau.</p><label className="field">Email<input type="email" required value={email} onChange={e=>setEmail(e.target.value)}/></label><label className="field">Mật khẩu<input type="password" minLength={8} required value={password} onChange={e=>setPassword(e.target.value)}/></label>{msg&&<div className="message">{msg}</div>}<button className="btn primary full" disabled={busy}>{busy?'Đang xử lý…':signup?'Đăng ký':'Đăng nhập'}</button><button type="button" className="btn ghost full" onClick={()=>{setSignup(!signup);setMsg('')}}>{signup?'Đã có tài khoản? Đăng nhập':'Chưa có tài khoản? Đăng ký'}</button><small className="muted">Chỉ dành cho người từ 18 tuổi.</small></form></div>
}

function Top({profile}:{profile:Profile}){return <header className="topbar"><div className="brand"><span>♥</span>Kết Nối</div><div className="user-mini"><div><strong>{profile.display_name}</strong><div className="muted">{profile.city||'Chưa đặt địa điểm'}</div></div><div className="avatar">{initials(profile.display_name)}</div></div></header>}
function Nav({view,setView}:{view:View;setView:(v:View)=>void}){return <aside className="side"><h3>Hẹn hò</h3><div className="nav"><N v="discover" t="Khám phá"/><N v="matches" t="Kết nối"/><N v="chat" t="Tin nhắn"/><N v="profile" t="Hồ sơ"/><button onClick={()=>void supabase.auth.signOut()}>Đăng xuất</button></div></aside>;function N({v,t}:{v:View;t:string}){return <button className={view===v?'active':''} onClick={()=>setView(v)}>{t}</button>}}
function MobileNav({view,setView}:{view:View;setView:(v:View)=>void}){return <nav className="mobile-nav">{([['discover','Khám phá'],['matches','Kết nối'],['chat','Chat'],['profile','Hồ sơ']] as [View,string][]).map(([v,t])=><button key={v} className={view===v?'active':''} onClick={()=>setView(v)}>{t}</button>)}</nav>}
...
function ProfileEditor({user,profile,onSaved,embedded=false}:{user:User;profile:Profile|null;onSaved:()=>void;embedded?:boolean}){
  ...
  const form=<form className="card" onSubmit={save}><h2 className="section-title">{embedded?'Hồ sơ của bạn':'Hoàn thiện hồ sơ'}</h2><div className="profile-grid"><div><div className="photo-preview">{photo?<img className="photo-preview" src={photo} alt="Ảnh hồ sơ"/>:<div className="photo-preview avatar">{initials(name)}</div>}</div><label className="field">Ảnh<input type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>{const f=e.target.files?.[0];if(f)void upload(f)}}/></label></div><div><label className="field">Tên hiển thị<input required value={name} onChange={e=>setName(e.target.value)}/></label><label className="field">Ngày sinh<input type="date" required value={birth} onChange={e=>setBirth(e.target.value)}/></label><label className="field">Giới tính<select value={gender} onChange={e=>setGender(e.target.value)}><option value="man">Nam</option><option value="woman">Nữ</option><option value="other">Khác</option></select></label><label className="field">Muốn gặp<select value={looking} onChange={e=>setLooking(e.target.value)}><option value="all">Tất cả</option><option value="man">Nam</option><option value="woman">Nữ</option></select></label><label className="field">Thành phố<input value={city} onChange={e=>setCity(e.target.value)}/></label><label className="field">Giới thiệu<textarea maxLength={500} value={bio} onChange={e=>setBio(e.target.value)}/></label></div></div>{msg&&<div className="message">{msg}</div>}<button className="btn primary" disabled={busy}>{busy?'Đang lưu…':'Lưu hồ sơ'}</button>{!embedded&&<button type="button" className="btn ghost" onClick={()=>void supabase.auth.signOut()}>Đăng xuất</button>}</form>
  return embedded?form:<div className="auth-wrap" style={{display:'block'}}><div style={{maxWidth:720,margin:'30px auto'}}>{form}</div></div>
}
...
function Discover({user,me}:{user:User;me:Profile}){
  ...
  if(loading)return <div className="card empty">Đang tìm người phù hợp…</div>
  if(!p)return <div className="card empty"><h2>Hết hồ sơ phù hợp lúc này</h2><p className="muted">Khi có thành viên mới, họ sẽ xuất hiện tại đây.</p><button className="btn secondary" onClick={()=>void load()}>Tải lại</button></div>
  return <>{notice&&<div className="message">{notice}</div>}<section className="card discover-card"><div className="hero">{p.photo?<img src={p.photo} alt={p.display_name}/>:<div className="fallback">{initials(p.display_name)}</div>}</div><div className="person"><h2>{p.display_name}, {age(p.birth_date)}</h2><div className="tags">{p.city&&<span className="tag">{p.city}</span>}<span className="tag">Đang tìm kết nối</span></div>{p.bio&&<p>{p.bio}</p>}</div><div className="actions"><button className="btn secondary" onClick={()=>void react(false)}>Bỏ qua</button><button className="btn primary" onClick={()=>void react(true)}>♥ Thích</button></div></section></>
}
...
function Matches({user,openChat}:{user:User;openChat:(m:Match)=>void}){
  ...
  if(loading)return <div className="card empty">Đang tải kết nối…</div>
  return <section><h2 className="section-title">Kết nối của bạn</h2><div className="list">{matches.length===0?<div className="card empty">Chưa có kết nối. Khi hai người cùng thích nhau, kết nối sẽ xuất hiện ở đây.</div>:matches.map(m=><button key={m.id} className="row" onClick={()=>openChat(m)}>{m.photo?<img className="avatar" src={m.photo} alt=""/>:<div className="avatar">{initials(m.other?.display_name)}</div>}<span className="grow"><strong>{m.other?.display_name||'Thành viên'}</strong><span className="muted">Nhấn để trò chuyện</span></span><span>›</span></button>)}</div></section>
}
...
function Chat({user,match}:{user:User;match:Match}){
  ...
  return <section><h2 className="section-title">{match.other?.display_name||'Tin nhắn'}</h2><div className="card chat"><div className="messages">{messages.map(m=><div key={m.id} className={`bubble ${m.sender_id===user.id?'mine':''}`}>{m.body}</div>)}</div><form className="composer" onSubmit={send}><input maxLength={1000} placeholder="Nhập tin nhắn…" value={text} onChange={e=>setText(e.target.value)}/><button className="btn primary">Gửi</button></form></div></section>
}
